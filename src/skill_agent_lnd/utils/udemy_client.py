...
def load_config():
    ...
    config_path = os.path.join(project_root, "udemy", "config.json")
    
    if not os.path.exists(config_path):
        config_path = os.path.abspath("udemy/config.json")
...
def _match_skill(query: str, title: str) -> bool:
    ...
    if q_raw in SKILL_ALIAS_MAP:
        search_term = SKILL_ALIAS_MAP[q_raw]
    else:
        search_term = q_raw
            ...
            for course in results:
                ...
                if _match_skill(skill_name, title):
                    ...
                    if language:
                        ...
                        course_lang = course_locale.get("locale", "") if isinstance(course_locale, dict) else ""
                    ...
                    return {
                        ...
                        "instructors": ", ".join([i.get("title") for i in course.get("visible_instructors", [])])
                    }
